...
func main() {
	...
	if os.Geteuid() != 0 {
		log.Fatal("zz-lan must run as root.")
	}

	var networkInterface string
	// 将命令行参数I存储到networkInterface中
	flag.StringVar(&networkInterface, "I", "", "Network interface name")
	flag.Parse()

	setupNetInfo(networkInterface)
...
//
//	// 具体实现：
//	// 首先通过listenARP等待在线主机返回arp报文,
//	// 当收到arp报文后，通过pushData方法，通过do管道将t计时器停止并将数据添加到data，并开启一个新的计时器
//
//	t = time.NewTicker(30 * time.Second) // 等待首个返回的报文的最长等待时间
//	for {
//		select {
//		case <-t.C:
//			PrintData()
